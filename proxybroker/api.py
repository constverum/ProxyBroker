...
class Broker:
    ...
    # async def check(self, data, types=None, countries=None, limit=None):
    #     self._checker.set_conditions(types=types) # , countries=countries
    #     self._countries = countries
    #     self._limit = limit
    #     await set_my_ip(self._timeout, self._loop)
    #     await self._load(data, push=self.push_to_check)

    #     self._tasks = asyncio.gather(
    #                     self._checker.check_judges(),
    #                     # self._checker.load(data)
    #                     )


    # async def _to_check(self, proxies):
    #     for host, port, *args in proxies.difference(self._allFoundProxyPairs):
    #         if host in self._allFoundProxyPairs:
    #             print('%s in self._allFoundProxyPairs' % host)
    #         p = await Proxy.create(host, port, *args)
    #         print('proxy created: %s' % p)
    #         if host != p.host:
    #             self._allFoundProxyPairs.add((host, p.port))
    #         if p.host:
    #             self._allFoundProxies.append(p)
    #             fut = asyncio.ensure_future(self._checker.check(p))
    #             fut.add_done_callback(lambda _: self._qToCheck.task_done())
    #             # self._futures.append(fut)
    #             self._qToCheck.put_nowait(fut)
    #             self._allFoundProxyPairs.add((p.host, p.port))

 # _push_to_result
        # while True:
        #     print('in cycle _recv_from_checker')
        #     p = await self.qChecked.get()
        #     print('after await self.qChecked.get()')
        #     # print('try put None')
        #     await self._qResult.put(p)
        #     self._update_limit()
        ...
        #     pass
            # if not self._futures.cancelled():
            #     print('\n\nfut:', self._futures, 'is fut.cancelled():', self._futures.cancelled(), 'is fut.done():', self._futures.done())
            #     print('try cancel: %s' % self._futures)
            #     self._futures.cancel()

                # print('\n\nfut:', fut, 'is fut.cancelled():', fut.cancelled(), 'is fut.done():', fut.done())
                    # print('try cancel: %s' % fut)
                    # print('\nfut:', fut, 'cancelled:', fut.cancelled(), 'done:', fut.done())
                    # fut.set_result(None)
            # print("Err: %s" % e)
            # print('X:', x)
            # for fut in tasks._children:
            #     print('\nFUT:', fut, 'is fut.cancelled():', fut.cancelled(), 'is fut.done():', fut.done())
    ...
    #     errors = Counter()
    #     [errors.update(p.errors) for p in proxies] # checker.proxies['clean']
    ...
    #     # lmb = lambda p: (len(p.host[:p.host.find('.')]), p.host[:3])
    #     # s5 = sorted(checker.get_proxies(types='SOCKS5'), key=lmb)
    #     # s4 = sorted(checker.get_proxies(types='SOCKS4'), key=lmb)
    #     # c = sorted(checker.get_proxies(types='CONNECT'), key=lmb)
    #     # hs = sorted(checker.get_proxies(types='HTTPS'), key=lmb)
    #     # h = sorted(checker.get_proxies(types='HTTP'), key=lmb)
    ...
    #           .format(
    #             all=len(proxies),
    #             # s5=s5, ls5=len(s5), s4=s4, ls4=len(s4),
    ...
    #         country = '{code}: {name:<20}'.format(
    #             code=p.geo['code'], name=p.geo['names']['en'])
