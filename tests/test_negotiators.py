...
@pytest.mark.parametrize('ngtr,check_anon_lvl,use_full_path', [
    ('SOCKS5', False, False),
    ('SOCKS4', False, False),
...
@pytest.mark.asyncio
@pytest.mark.parametrize('ngtr,port,recv,expected', [
    ('SOCKS5', 80,
    ...
    ('SOCKS5', 443,
     ...
     [call(b'\x05\x01\x00'), call(b'\x05\x01\x00\x01\x7f\x00\x00\x01\x01\xbb')]),  # noqa
    ('SOCKS4', 80,
...
@pytest.mark.asyncio
@pytest.mark.parametrize('ngtr,recv,expected', [
    # wrong response:
    ('SOCKS5', future_iter(b'\x05\xff'), [call(b'\x05\x01\x00')]),
    ('SOCKS4', future_iter(b'HTTP/1.1 400 Bad Request'), [call(b'\x04\x01\x00P\x7f\x00\x00\x01\x00')]),  # noqa
    # failed to connect:
    ('SOCKS5', future_iter(b'\x05\x00', b'\x05\x05'), [call(b'\x05\x01\x00'), call(b'\x05\x01\x00\x01\x7f\x00\x00\x01\x00P')]),  # noqa
    ('SOCKS4', future_iter(b'\x00['), [call(b'\x04\x01\x00P\x7f\x00\x00\x01\x00')]),  # noqa
])
...
@pytest.mark.asyncio
@pytest.mark.parametrize('ngtr,port,recv', [
    ('CONNECT:80', 80, future_iter(b'HTTP/1.1 200 Connection established\r\n\r\n')),  # noqa
    ('CONNECT:25', 25, future_iter(b'HTTP/1.1 200 Connection established\r\n\r\n', b'220 smtp2.test.com')),  # noqa
    ('HTTPS', 443, future_iter(b'HTTP/1.1 200 Connection established\r\n\r\n')),  # noqa
])
